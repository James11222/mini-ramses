...
subroutine kick_drift_part(s,p,ilevel,action_part)
  !$acc routine(open_cache) seq
  !$acc routine(get_parent_cell) seq
  ...
  call nvtxStartRange("The big particles loop", 0) 
  ! Loop over particles

  !$acc parallel loop private(x,dd,dg,ig,id,vol,ckey,icell,hash_nbor,ff,gridp) 
