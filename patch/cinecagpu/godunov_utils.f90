...
subroutine riemann_llf(qleft,qright,fgdnv,gamma,gamma_rad,smallr,smallc)
  ...
  ! dummy arguments
  real(dp),intent(in)::gamma,smallr,smallc
  real(dp),dimension(1:nener),intent(in)::gamma_rad
  real(dp),dimension(1:nvar),intent(in)::qleft,qright
  real(dp),dimension(1:nvar+1),intent(out)::fgdnv
