...
subroutine unsplit(uin,gravin,qin,cin,flux,tmp,dq,qm,qp,fx,tx,divu,&
  ...
  call nvtxStartRange("save flux X", 6)
  ...
  call nvtxEndRange
  ! Save flux in output array
  call nvtxStartRange("save flux Y", 6)
  !$acc parallel loop collapse(4)
  do ivar=1,nvar
     do k=klo,khi
        do j=jf1,jf2
  ...
  call nvtxEndRange
  ! Save flux in output array
  call nvtxStartRange("save flux Z", 6)
  !$acc parallel loop collapse(4)
  do ivar=1,nvar
     do k=kf1,kf2
        do j=jlo,jhi
...
#if NENER>0
  !$acc parallel loop collapse(3) private(e,dex,se0)
#else
...
#if NENER>0
  !$acc parallel loop collapse(3) private(e,dex,dey,se0)
#else
  !$acc parallel loop collapse(3)
#endif
  do k = klo, khi
...
#if NENER>0
  !$acc parallel loop collapse(3) private(e,dex,dey,dez,se0)
#else
  !$acc parallel loop collapse(3)
#endif
  do k = klo, khi
...
subroutine cmpflxm(qm,im1,im2,jm1,jm2,km1,km2, &
  ...
  !$acc routine(riemann_llf) seq
  !$acc routine(riemann_hllc) seq
  !$acc routine(riemann_hll) seq
  use amr_parameters, only: dp, ndim
...
!  do concurrent (k=klo:khi,j=jlo:jhi,i=ilo:ihi) local(fgdnv)
!$acc parallel loop collapse(3) private(fgdnv,qleft,qright)
  do k = klo, khi
     do j = jlo, jhi
        do i = ilo, ihi
