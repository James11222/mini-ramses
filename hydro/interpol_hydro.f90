...
subroutine upload_fine(ilevel)
  ...
  integer::ind,ivar,igrid,icell,idim,ilev
  ...
  do ioct=head(ilevel),tail(nlevelmax-1)
     do ivar=1,nvar
        do ind=1,twotondim
  ...
  do ilev=nlevelmax-1,ilevel,-1
     if(noct_tot(ilev)==0 .or. noct_tot(ilev+1)==0)cycle

     call open_cache(operation_upload,domain_decompos_amr)

     ! Loop over finer level grids
     hash_key(0)=ilev+1
     do ioct=head(ilev+1),tail(ilev+1)
