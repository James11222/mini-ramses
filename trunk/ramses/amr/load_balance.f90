...
!        if(myid==1)write(*,'("iter=",I4,1X,17(I10,1X))')iter,npart_cum
        ...
        do while (unbalance.GT.1)
           ...
           do i=headp(ilev),tailp(ilev)
              ...
              ix_ref(1:ndim) = int(xp(ipart,1:ndim)/(2*dx_loc))
              call hilbert_key(ix_ref,hk_ref,ilev-1)
  ...
  do ilev=ilevel,nlevelmax
     ...
     do ipart=headp(ilev),tailp(ilev)
        ...
        if(.NOT. ALL(ix.EQ.ix_ref))then
           ...
           ! Check if grid sits outside future processor boundaries
           if(    gt_keys(bound_key_part(1:nhilbert,myid-1,ilev),hk_ref(1:nhilbert)).OR. &
                & ge_keys(hk_ref(1:nhilbert),bound_key_part(1:nhilbert,myid,ilev)))then              
              ! Determine the future processor
              do icpu=1,ncpu
                 if(    ge_keys(hk_ref(1:nhilbert),bound_key_part(1:nhilbert,icpu-1,ilev)).AND. &
                      & gt_keys(bound_key_part(1:nhilbert,icpu,ilev),hk_ref(1:nhilbert)))then
                    grid_cpu=icpu
                 end if
              end do
           endif
...
!#########################################################################
!#########################################################################
subroutine balance_part_vec(ilevel)
  use amr_commons
  use pm_commons
  use hilbert
  implicit none
  ...
  integer(kind=4), dimension(1:nvector),save::grid_cpu
...
!        if(myid==1)write(*,'("iter=",I4,1X,17(I10,1X))')iter,npart_cum
        ...
        do while (unbalance.GT.1)
           ...
           ! Loop over particles in Hilbert order
           do ip=headp(ilev),tailp(ilev),nvector

              ngrid=MIN(nvector,tailp(ilev)-ip+1)

              ! Compute Hilbert key of particle parent grid
              do idim=1,ndim
                 do i=1,ngrid
                    ipart=sortp(ip+i-1)
                    ix_ref(i,idim) = int(xp(ipart,idim)/(2*dx_loc))
                 end do
              end do
              call hilbert_key_vec(ix_ref,hk_ref,dummy_state,0,ilev-1,ngrid)
              
              ! Determine in which processor the grid sits
              do icpu=1,ncpu
                 do i=1,ngrid
                    if(gt_keys(bound_key_target(1:nhilbert,icpu),hk_ref(i,1:nhilbert)))then
                       npart_cum(icpu)=npart_cum(icpu)+1
                    end if
                 end do
              end do
...
!              unbalance=MAX(unbalance,ABS(bound_key_right(1,icpu)-bound_key_left(1,icpu)))
  ...
  do ilev=ilevel,nlevelmax
     ...
     ! Loop over particles
     do ipart=headp(ilev),tailp(ilev),nvector

        ngrid=MIN(nvector,tailp(ilev)-ipart+1)

        ! Determine in which cpu particle should sit.
        do idim=1,ndim
           do i=1,ngrid
              ix_ref(i,idim) = int(xp(ipart+i-1,idim)/(2*dx_loc))
           end do
        end do

        grid_cpu(1:ngrid) = myid
        ...
        ! Check if grid sits outside future processor boundaries
        do i=1,ngrid
           if(    gt_keys(bound_key_part(1:nhilbert,myid-1,ilev),hk_ref(i,1:nhilbert)).OR. &
                & ge_keys(hk_ref(i,1:nhilbert),bound_key_part(1:nhilbert,myid,ilev)))then              
              ! Determine the future processor
              do icpu=1,ncpu
                 if(    ge_keys(hk_ref(i,1:nhilbert),bound_key_part(1:nhilbert,icpu-1,ilev)).AND. &
                      & gt_keys(bound_key_part(1:nhilbert,icpu,ilev),hk_ref(i,1:nhilbert)))then
                    grid_cpu(i)=icpu
                 end if
              end do
           endif
        ...
        do i=1,ngrid
           if(grid_cpu(i).EQ.myid)then
              ! Update count for local cpu
              count_loc=count_loc+1
           else
              ! Update count for remote cpu
              send_cnt(grid_cpu(i))=send_cnt(grid_cpu(i))+1
           end if
        end do
     ...
     do ipart=headp(ilev),tailp(ilev),nvector
        ...
        do i=1,ngrid
           if(grid_cpu(i).EQ.myid)then
              ! Update count for local cpu
              count_loc=count_loc+1
              workp(ipart+i-1)=headp(ilevel)-1+count_loc
           else
              ! Update count for remote cpu
              send_cnt(grid_cpu(i))=send_cnt(grid_cpu(i))+1
              workp(ipart+i-1)=offset_cpu(grid_cpu(i))+send_cnt(grid_cpu(i))
           end if
        end do
...
end subroutine balance_part_vec
!##############################################################################
!##############################################################################
!##############################################################################
!##############################################################################
