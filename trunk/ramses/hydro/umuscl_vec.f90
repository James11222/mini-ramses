...
! ----------------------------------------------------------------
subroutine unsplit_vec(uin,gravin,flux,tmp,dx,dy,dz,dt,ngrid)
  use amr_parameters
  use const             
  ...
  call ctoprim_vec(uin,qin,cin,gravin,dt,ngrid)

  ! Compute TVD slopes
  call uslope_vec(qin,dq,dx,dt,ngrid)

  ! Compute 3D traced-states in all three directions
  if(scheme=='muscl')then
#if NDIM==1
     call trace1d_vec(qin,dq,qm,qp,dx      ,dt,ngrid)
#endif
#if NDIM==2
     call trace2d_vec(qin,dq,qm,qp,dx,dy   ,dt,ngrid)
#endif
#if NDIM==3
     call trace3d_vec(qin,dq,qm,qp,dx,dy,dz,dt,ngrid)
#endif
  endif

  ! Solve for 1D flux in X direction
  call cmpflxm_vec(qm,iu1+1,iu2+1,ju1  ,ju2  ,ku1  ,ku2  , &
...
#if NDIM>1
  call cmpflxm_vec(qm,iu1  ,iu2  ,ju1+1,ju2+1,ku1  ,ku2  , &
       &       qp,iu1  ,iu2  ,ju1  ,ju2  ,ku1  ,ku2  , &
...
#if NDIM>2
  call cmpflxm_vec(qm,iu1  ,iu2  ,ju1  ,ju2  ,ku1+1,ku2+1, &
       &       qp,iu1  ,iu2  ,ju1  ,ju2  ,ku1  ,ku2  , &
  ...
  if(difmag>0.0)then
    call cmpdivu_vec(qin,divu,dx,dy,dz,ngrid)
    call consup_vec(uin,flux,divu,dt,ngrid)
  endif

end subroutine unsplit_vec
!###########################################################
!###########################################################
!###########################################################
!###########################################################
subroutine trace1d_vec(q,dq,qm,qp,dx,dt,ngrid)
  use amr_parameters
  use hydro_parameters
  use hydro_vec_parameters
  use const
...
end subroutine trace1d_vec
!###########################################################
!###########################################################
!###########################################################
!###########################################################
#if NDIM>1
subroutine trace2d_vec(q,dq,qm,qp,dx,dy,dt,ngrid)
  use amr_parameters
  use hydro_parameters
  use hydro_vec_parameters
  use const
...
!###########################################################
#if NDIM>2
subroutine trace3d_vec(q,dq,qm,qp,dx,dy,dz,dt,ngrid)
  use amr_parameters
  use hydro_parameters
  use hydro_vec_parameters
  use const
...
end subroutine trace3d_vec
#endif
!###########################################################
!###########################################################
!###########################################################
!###########################################################
subroutine cmpflxm_vec(qm,im1,im2,jm1,jm2,km1,km2, &
     ...
     do j = jlo, jhi
        do i = ilo, ihi
           ...
           if(riemann.eq.'acoustic')then
              ...
              call riemann_llf_vec (qleft,qright,fgdnv,ngrid)
           else if (riemann.eq.'hllc')then
              call riemann_hll_vec (qleft,qright,fgdnv,ngrid)
...
end subroutine cmpflxm_vec
!###########################################################
!###########################################################
!###########################################################
!###########################################################
subroutine ctoprim_vec(uin,q,c,gravin,dt,ngrid)
  use amr_parameters
  use hydro_parameters
  use hydro_vec_parameters
  use const
...
end subroutine ctoprim_vec
!###########################################################
!###########################################################
!###########################################################
!###########################################################
subroutine uslope_vec(q,dq,dx,dt,ngrid)
  use amr_parameters
  use hydro_parameters
  use hydro_vec_parameters
  use const
